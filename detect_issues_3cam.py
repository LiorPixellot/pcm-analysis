#!/usr/bin/env python3
"""
Detect Issues Script — 3-Camera variant

Reads PQS_blur_by_venue.xlsx and processes one event per venue (first event with
a focus dir), using Gemini to detect camera issues in 3-camera setups
(CAM0=right, CAM1=center, CAM2=left).

Analyzes focus issues per adjacent pair: (CAM0, CAM1) and (CAM1, CAM2).
Gracefully falls back to 2-camera analysis when CAM2 is absent.
"""
...
DETECT_ISSUES_3CAM_PROMPT = """Analyze the following camera images for focus issues.

This is a 3-camera setup covering a sports venue:
- CAM0 = right camera
- CAM1 = center camera
- CAM2 = left camera

Adjacent camera pairs share overlapping coverage:
- Pair (0,1): CAM0 and CAM1 overlap on the right side of the venue
- Pair (1,2): CAM1 and CAM2 overlap on the left side of the venue

FOCUS DETECTION — check for ALL of these:

1. SHARPNESS DIFFERENCE BETWEEN ADJACENT CAMERAS:
   Compare edges, field lines, text, and fine details between each adjacent pair.
   If one camera is noticeably sharper/crisper than its neighbor, that is a focus issue.
   Report which pair is affected and which camera is worse.
...
3. OVERALL CAMERA QUALITY:
   Check if any individual camera has uniformly bad focus (entirely soft/blurry).

Return ONLY valid JSON with exactly this structure:
{
    "observation": "Describe sharpness comparison between camera pairs and any focus transitions",
    "has_issue": "Yes" or "No",
    "issue_type": "Focus" or "None",
    "affected_cameras": "CAM0" or "CAM1" or "CAM2" or "CAM0,CAM1" or "CAM1,CAM2" or "All" or "None",
    "pair_01_issue": "Yes" or "No",
    "pair_12_issue": "Yes" or "No"
}

Rules:
- differences in exposure or color between cameras are NOT focus issues — only sharpness/blur differences count
- If all cameras are equally soft or blurred, that is still a focus issue (affected_cameras = "All")
- pair_01_issue: "Yes" if there is a sharpness difference between CAM0 and CAM1
- pair_12_issue: "Yes" if there is a sharpness difference between CAM1 and CAM2
- affected_cameras: list the specific cameras that have focus problems
- Return ONLY valid JSON, no markdown formatting or extra text"""

DETECT_ISSUES_2CAM_PROMPT = """Analyze the following camera images for focus issues.
...
Return ONLY valid JSON with exactly this structure:
{
    ...
    "has_issue": "Yes" or "No",
    "issue_type": "Focus" or "None",
    "affected_cameras": "CAM0" or "CAM1" or "Both" or "None",
    "pair_01_issue": "Yes" or "No",
    "pair_12_issue": "N/A"
}
...
# Reuse the same example categories and labels from detect_issues.py
...
def analyze_venue(client: genai.Client, model_name: str,
                  ...
                  cam2_bytes: Optional[bytes] = None,
    ...
    """Build parts list with prompt + examples + venue images, call Gemini.

    Selects the 3-cam or 2-cam prompt based on whether cam2_bytes is provided.
    ...
    prompt = DETECT_ISSUES_3CAM_PROMPT if is_3cam else DETECT_ISSUES_2CAM_PROMPT
    ...
    generation_config = types.GenerateContentConfig(
        ...
        labels={"job": "detect-issues-3cam", "pipeline": "pcm"},
...
def process_entry(index: int, entry: dict, client: genai.Client, model_name: str,
    ...
    if images is None:
        ...
        result_row["affected_cameras"] = "None"
        result_row["pair_01_issue"] = "N/A"
        result_row["pair_12_issue"] = "N/A"
    ...
    try:
        result, token_usage = analyze_venue(
            client, model_name, cam0_bytes, cam1_bytes, cam2_bytes, joined_bytes,
        ...
        result_row["affected_cameras"] = result.get("affected_cameras", "None") or "None"
        result_row["pair_01_issue"] = result.get("pair_01_issue", "No") or "No"
        result_row["pair_12_issue"] = result.get("pair_12_issue", "N/A") or "N/A"
        ...
        result_row["issue_type"] = "None"
        result_row["affected_cameras"] = "None"
        result_row["pair_01_issue"] = "Error"
        result_row["pair_12_issue"] = "Error"
        return {"index": index, "result_row": result_row, "token_usage": None,
                "processed": False, "skipped": True, "error": str(e), "traceback": tb}


def main():
    parser = argparse.ArgumentParser(description="Detect camera issues using Gemini — 3-camera variant (xlsx-driven, one event per venue)")
    parser.add_argument("--dataset", type=str, required=True, help="Data directory path (e.g. data/s3_23)")
    ...
    fieldnames = ["venue_id", "event_id", "has_issue", "issue_type", "affected_cameras", "pair_01_issue", "pair_12_issue"]
            ...
            for future in concurrent.futures.as_completed(future_to_wi):
                ...
                if res.get("skipped"):
                    ...
                    print(f"[{completed}/{total_items}] {venue_id}/{event_id}"
                          ...
                          f", cameras: {result_row['affected_cameras']}"
                          f", pair01: {result_row['pair_01_issue']}"
                          f", pair12: {result_row['pair_12_issue']}"
    ...
    with open(cost_path, 'w') as f:
        f.write("DETECT ISSUES (3-CAMERA) - COST REPORT\n")
        f.write("=" * 50 + "\n\n")

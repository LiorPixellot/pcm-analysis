...
Provide a JSON response with exactly this structure:
{
    ...
    "focus_comparison": "Left sharper" or "Right sharper" or "Similar" or "N/A"
}
...
def load_images(focus_dir: Path) -> Optional[Tuple[bytes, bytes, Optional[bytes]]]:
    ...
    try:
        ...
        if joined_path.exists():
            try:
                joined_image = Image.open(joined_path)
                joined_bytes = pil_to_bytes(joined_image)
            except Exception as e:
...
def process_single_row(index: int, row: dict, data_base: Path, client: genai.Client,
    ...
    if not focus_dir.exists():
        ...
        result_row["focus_comparison"] = "N/A"
    ...
    try:
        ...
        if is_measurable == "No":
            ...
            result_row["focus_comparison"] = result.get("focus_comparison", "N/A") or "N/A"
